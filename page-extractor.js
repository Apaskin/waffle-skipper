...
(function () {
    ...
    for (var k = 0; k < keyCandidates.length; k++) {
      ...
      for (var v = 0; v < requestVariants.length; v++) {
        try {
          ...
          if (tracks.length > 0) {
            console.log('[Waffle Skipper Extractor] Innertube returned', tracks.length, 'tracks');
  ...
  async function tryCaptureFromTrackList(videoId, tracks, methodName) {
    ...
    for (var t = 0; t < orderedTracks.length; t++) {
      ...
      if (parsed && parsed.events && parsed.events.length > 0) {
        console.log('[Waffle Skipper Extractor] Captured transcript via ' + methodName + ' for', videoId + ':', parsed.events.length, 'events');
        capturedTranscripts[videoId] = parsed;
